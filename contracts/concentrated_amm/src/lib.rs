...
impl ConcentratedAmm {
    pub fn initialize(
        ...
        e.storage()
            .instance()
            .set(&DataKey::CurrentSqrtPrice, &initial_sqrt_price_x64);
    ...
    pub fn mint(
        ...
        let current_sqrt_price: u128 = e
            .storage()
            .instance()
            .get(&DataKey::CurrentSqrtPrice)
            .unwrap();
        ...
        if current_sqrt_price <= sqrt_ratio_ax64 {
            ...
            let den = sqrt_ratio_bx64
                .checked_sub(sqrt_ratio_ax64)
        ...
        let flipped_lower = Self::update_tick(
            &e,
            tick_lower,
            current_tick,
            liquidity as i128,
        ...
        let flipped_upper = Self::update_tick(
            &e,
            tick_upper,
            current_tick,
            liquidity as i128,
        ...
        if flipped_lower {
            Self::flip_tick_bitmap(&e, tick_lower, tick_spacing);
        }
        if flipped_upper {
            Self::flip_tick_bitmap(&e, tick_upper, tick_spacing);
        }

        let (fee_growth_inside_0, fee_growth_inside_1) =
            Self::get_fee_growth_inside(&e, tick_lower, tick_upper, current_tick, fg0, fg1);
        ...
        // Update tokens owed
        pos.tokens_owed_0 += mul_div_u128(
            pos.liquidity,
            fee_growth_inside_0.wrapping_sub(pos.fee_growth_inside_0_last),
            Q64,
        )
        .unwrap_or(0);
        pos.tokens_owed_1 += mul_div_u128(
            pos.liquidity,
            fee_growth_inside_1.wrapping_sub(pos.fee_growth_inside_1_last),
            Q64,
        )
        .unwrap_or(0);
        ...
        if current_tick >= tick_lower && current_tick < tick_upper {
            ...
            e.storage()
                .instance()
                .set(&DataKey::Liquidity, &(current_liq + liquidity));
        }
    ...
    pub fn swap(
        ...
        let mut current_sqrt_price: u128 = e
            .storage()
            .instance()
            .get(&DataKey::CurrentSqrtPrice)
            .unwrap();
        ...
        while amount_remaining > 0 {
            if liquidity == 0 {
                ...
                let (next_tick, initialized) = Self::next_initialized_tick_within_one_word(
                    &e,
                    current_tick,
                    tick_spacing,
                    zero_for_one,
                ...
                if let Some(mut tick_info) = e.storage().persistent().get::<_, TickInfo>(&tick_key)
                {
                    tick_info.fee_growth_outside_0 =
                        fg0.wrapping_sub(tick_info.fee_growth_outside_0);
                    tick_info.fee_growth_outside_1 =
                        fg1.wrapping_sub(tick_info.fee_growth_outside_1);
            ...
            let amount_remaining_less_fee =
                mul_div_u128(amount_remaining, 10000 - (fee_bps as u128), 10000)?;
            ...
            let amount_out_step = if crossed {
                ...
                let next_p = get_next_sqrt_price_from_input(
                    current_sqrt_price,
                    liquidity,
                    amount_in_step,
                    zero_for_one,
            ...
            if crossed {
                ...
                let mut tick_info: TickInfo = e
                    .storage()
                    .persistent()
                    .get(&DataKey::Tick(next_tick))
                    .unwrap();
                ...
                e.storage()
                    .persistent()
                    .set(&DataKey::Tick(next_tick), &tick_info);
                ...
                current_sqrt_price = get_next_sqrt_price_from_input(
                    current_sqrt_price,
                    liquidity,
                    amount_in_step,
                    zero_for_one,
        ...
        e.storage()
            .instance()
            .set(&DataKey::CurrentSqrtPrice, &current_sqrt_price);
        e.storage()
            .instance()
        ...
        client_in.transfer(
            ...
            &(amount_in_total as i128),
        );
        client_out.transfer(
            &e.current_contract_address(),
            &to,
            &(amount_out_total as i128),
        );
    ...
    fn update_tick(
        ...
        let flipped = (liquidity_gross_before == 0 && info.liquidity_gross > 0)
            || (liquidity_gross_before > 0 && info.liquidity_gross == 0);
    ...
    fn get_fee_growth_inside(
        ...
        let lower_info: TickInfo = e
            .storage()
            .persistent()
            .get(&DataKey::Tick(tick_lower))
            .unwrap_or(TickInfo {
                liquidity_gross: 0,
                liquidity_net: 0,
                fee_growth_outside_0: 0,
        ...
        let upper_info: TickInfo = e
            .storage()
            .persistent()
            .get(&DataKey::Tick(tick_upper))
            .unwrap_or(TickInfo {
                liquidity_gross: 0,
                liquidity_net: 0,
                fee_growth_outside_0: 0,
        ...
        let (fee_below_0, fee_below_1) = if tick_current >= tick_lower {
            ...
            (
                fg0.wrapping_sub(lower_info.fee_growth_outside_0),
                fg1.wrapping_sub(lower_info.fee_growth_outside_1),
            )
        };
        ...
        let (fee_above_0, fee_above_1) = if tick_current < tick_upper {
            ...
            (
                fg0.wrapping_sub(upper_info.fee_growth_outside_0),
                fg1.wrapping_sub(upper_info.fee_growth_outside_1),
            )
        };
